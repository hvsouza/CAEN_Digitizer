...
static void SetDefaultConfiguration(WaveDumpConfig_t *WDcfg) {
    ...
    WDcfg->ContinuousMax = 0; // added by Henrique Souza
...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	while(!feof(f_ini)) {
        ...
        // Get maximum number of waveforms to be saved each time 'W' is pressed
        if (strstr(str, "CONTINUOUS_WRITTING_MAX")!=NULL){
            read = fscanf(f_ini, "%lu", &WDcfg->ContinuousMax);
            continue;
        }
